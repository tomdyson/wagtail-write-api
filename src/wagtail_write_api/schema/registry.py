...
class SnippetSchemaRegistry:
    ...
    def register(self, model_class):
        key = f"{model_class._meta.app_label}.{model_class.__name__}"
        self._schemas[key] = generate_schemas_for_model(model_class, model_kind="snippet")
    ...
    def auto_discover(self):
        ...
        exclude = set(api_settings.EXCLUDE_SNIPPET_TYPES)
    ...
    def all_snippet_types(self) -> list[str]:
        self._ensure_discovered()
        return list(self._schemas.keys())
...
snippet_schema_registry = SnippetSchemaRegistry()
